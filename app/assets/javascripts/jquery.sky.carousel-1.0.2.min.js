...
web.Carousel.prototype={init:function(){this.initDOM();this.initConfigParams();this.initEvents();this.initContentWrapper();this.initContainer();this.initGradientOverlays();this.initNavigationButtons();this.initResizeListener();this.initKeyboardNavigation();this.initMouseWheelSupport();this.initAutoSlideshow();this.calculateUnits();this.update();this.dom.carousel.css("visibility","visible")},initDOM:function(){var a=this.settings;this.dom.document=$(document);this.dom.wrapper=this.dom.carousel.children(".web-carousel-wrapper");
this.dom.container=this.dom.wrapper.children(".web-carousel-container");this.dom.items=this.dom.container.children("li");this.dom.links=this.dom.container.find("li > a");this.dom.images=this.dom.container.find("li img");this.dom.carousel.addClass("sc-no-select");a.preload&&!1!=$.support.leadingWhitespace&&(this.dom.wrapper.css({visibility:"hidden",opacity:0}),a.showPreloader&&(this.preloader=$('<div class="sc-preloader"></div>'),this.dom.carousel.append(this.preloader)));this.dom.images.each(function(){$(this).addClass("sc-image");
this.ondragstart=function(){return!1}})},initConfigParams:function(){var a=this.settings,b=parseInt(this.dom.items.css("padding-left")),c=parseInt(this.dom.items.css("border-left-width")),d=parseInt(this.dom.images.css("padding-left")),g=parseInt(this.dom.images.css("border-left-width"));a.itemWidth+=2*(b+c+d+g);a.itemHeight+=2*(b+c+d+g);web.Utils.has2dTransformationSupport()&&this.dom.items.css(web.Utils.getPrefixedProperty("transformOrigin"),"center "+Math.round(a.itemHeight/2)+"px")},initEvents:function(){web.Utils.hasTouchSupport()?
(this.events.startEvent="touchstart.sc",this.events.moveEvent="touchmove.sc",this.events.endEvent="touchend.sc",this.events.touchCancel="touchcancel.sc"):(this.events.startEvent="mousedown.sc",this.events.moveEvent="mousemove.sc",this.events.endEvent="mouseup.sc")},initContainer:function(){var a=this,b=0;this.container=new web.Container(this.dom.container,this);this.dom.items.each(function(c){c=new web.CarouselItem($(this),a);c.subscribe("load",function(c){++b;if(b==a.dom.items.length)a.onAllLoaded()});
c.load();a.carouselItems.push(c)});this.dom.carousel.on(this.events.startEvent,function(b){a.onStart(b)});web.Utils.hasTouchSupport()||this.dom.carousel.hover(function(b){a.mouseOver=!0;a.updateCursor()},function(b){a.mouseOver=!1;a.updateCursor()});this.selectedItem=this.getStartItem();this.selectedItem.addClass("sc-selected");this.updatePlugin()},initGradientOverlays:function(){var a=this.settings;if(a.gradientOverlayVisible){var b=this.createGradientOverlay("left",a.gradientStartPoint,a.gradientEndPoint,
...
d.setX(d.getBaseOffset()+g);d.setScale(a.unselectedItemZoomFactor)}this.selectedItem.setScale(a.selectedItemZoomFactor);this.container.setX(this.centerX-this.selectedItem.x-a.itemWidth/2)},updateCursor:function(){this.dragging?web.Utils.setCursor("closedhand"):this.mouseOver?web.Utils.setCursor("openhand"):web.Utils.setCursor("auto")},calculateAlpha:function(a){return(this.settings.motionStartDistance-a)*this.alphaUnit+this.settings.unselectedItemAlpha},calculateScale:function(a){return(this.settings.motionStartDistance-
a)*this.scaleUnit+this.settings.unselectedItemZoomFactor},calculateExtraDistance:function(a){return Math.ceil((this.settings.motionStartDistance-a)*this.extraDistanceUnit)},onStart:function(a){function b(a){var b=a.originalEvent,c=web.Utils.hasTouchSupport()?b.touches[0].clientX:a.clientX,g=web.Utils.hasTouchSupport()?b.touches[0].clientY:a.clientY,r=(c-startX)/2+n;m=f-c;f=c;if(web.Utils.hasTouchSupport()){if(1<b.touches.length){q=!0;return}t||(t=!0,Math.abs(g-h)>Math.abs(c-startX)+5?d.isScrolling=
!0:d.isScrolling=!1);if(d.isScrolling){q=!0;return}}a.preventDefault();r>p+40&&(r=p+40+0.2*(r-p));r<l-40&&(r=l-40-0.2*(l-r));d.dragging||(d.dragging=!0,d.updateCursor());d.targetLeft=r}function c(a){var f=a.originalEvent,e=web.Utils.hasTouchSupport()?f.changedTouches[0].clientX:a.clientX;a=web.Utils.hasTouchSupport()?f.changedTouches[0].clientY:a.clientY;var f=Math.round(m/20),h=d.closestItem.index();d.timer&&d.timer.start();d.dom.document.off(d.events.moveEvent,b);d.dom.document.off(d.events.endEvent,
